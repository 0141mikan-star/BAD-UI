...
# --- モード2: GOOD UI ゲーム (天国) ---
    ...
    st.write("巨大なバツボタンと、優しく動く広告。ストレスフリーな体験です。")
    ...
    <div id="gf" style="position:relative; width:100%; height:550px; background:#e8f0fe; border:3px solid #1a73e8; border-radius:12px; overflow:hidden; user-select:none;">
        ...
        <div id="gc" style="
            ...
            animation: moveGood 15s infinite ease-in-out;
        ">
            <div style="padding: 40px 20px 20px 20px; text-align:center;">
                ...
                <h2 id="gtit" style="color:#1a73e8; margin:10px 0; font-size:20px;">【天草方面】海釣り公園</h2>
                <p id="gdes" style="font-size:12px; color:#555; line-height:1.4;">
                    トイレや売店完備で安心。手軽に色々な魚が釣れる、<br>家族連れに最適なスポット。
                </p>
                <div id="gimg" style="width:100%; height:160px; background:#fff8e1; margin:15px 0; display:flex; align-items:center; justify-content:center; border:1px dashed #1a73e8; border-radius:5px;">
            ...
            <div id="gtgt" style="
                position:absolute; width:70px; height:70px; background:#fff; color:#1a73e8; border:4px solid #1a73e8;
    ...
    <script>
        ...
        gsb.onclick=()=>{{
            gsc=0; gt=30; gse.innerText=gsc; gte.innerText=gt; gsb.style.display='none'; gc.style.display='block'; change();
